 ...
 * @author yhy
 */
public class ExportProfitExcel {
    //打印商品销售信息
    public static void printSale(List<ViewProfit> psalelist,File file){
       //1 ============设定表列名
        String totalprofit = "利润";
        String themonth = "月份";
            try {
           ...
           //创建字体，7个参数分别是字体名称，字号，是否粗体，是否斜体，下划线，颜色，上下标
            ...
            //3 ============设置第一行为列名,并添加列名
            Label labelProfit= new Label(0, 0, totalprofit, format); //写入字符串，确定位置（列，行）、内容和格式
            Label labelmonth = new Label(1, 0, themonth, format);
            //添加列名
            sheet1.addCell(labelProfit); //sheet对象添加文本的数据类型为Label
            sheet1.addCell(labelmonth);
            
            //4 =====================取出List集合中的的内容，放到Excel中
            int row = psalelist.size();//获取集合的长度作为表格的行数
            for (int i = 0; i < row; i++) {
                //获取每一行数据，从第一行开始，第0行是标题
                Label profit1 = new Label(0, i+1, psalelist.get(i).getTotalprofit().toString(), format1);
                Label month1 = new Label(1, i+1, psalelist.get(i).getThemonth().toString(), format1);
                
                //添加每一行内容
                sheet1.addCell(profit1);
                sheet1.addCell(month1);
