 ...
 * @author LIUYIYU
 */
public class ExportExcelSelreturn {

    public static void printSelreturn(List<ViewSelReturn> srtlist, File file) {
        //1 ============设定表列名
        String prtid = "退货编号";
        String pname = "商品名称";
        String ptype = "商品型号";
        String supplier = "客户姓名";
        ...
        try {
            ...
            //3 ============设置第一行为列名,并添加列名
            Label labelSrtid = new Label(0, 0, prtid, format); //写入字符串，确定位置（列，行）、内容和格式
            Label labelProname = new Label(1, 0, pname, format);
            Label labelPtype = new Label(2, 0, ptype, format);
            Label labelCustomer = new Label(3, 0, supplier, format);
            Label labelSrtprice = new Label(4, 0, prtprice, format);
            Label labelSrtcount = new Label(5, 0, prtcount, format);
            Label labelSrtdate = new Label(6, 0, prtdate, format);
            //添加列名
            sheet1.addCell(labelSrtid); //sheet对象添加文本的数据类型为Label
            sheet1.addCell(labelProname);
            sheet1.addCell(labelPtype);
            sheet1.addCell(labelCustomer);
            sheet1.addCell(labelSrtprice);
            sheet1.addCell(labelSrtcount);
            sheet1.addCell(labelSrtdate);
            //4 =====================取出List集合中的的内容，放到Excel中
            int row = srtlist.size();//获取集合的长度作为表格的行数
            for (int i = 0; i < row; i++) {
                //获取每一行数据，从第一行开始，第0行是标题
                Label rtid = new Label(0, i + 1, srtlist.get(i).getSrtid() + "", format1);
                Label name = new Label(1, i + 1, srtlist.get(i).getProname(), format1);
                Label type = new Label(2, i + 1, srtlist.get(i).getType(), format1);
                Label cus = new Label(3, i + 1, srtlist.get(i).getCusname(), format1);
                Label price = new Label(4, i + 1, srtlist.get(i).getSrtprice().toString(), format1);
                Label count = new Label(5, i + 1, srtlist.get(i).getSrtcount() + "", format1);
                Label date = new Label(6, i + 1, srtlist.get(i).getSrtdate().toString(), format1);
